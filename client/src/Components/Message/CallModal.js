...
import RingRing from '../../static/audios/phonering.mp3';
...
const CallModal = () => {
    ...
    // End Call
    const addCallMessage = useCallback((data, times, disconnect) => {
        if(call.recipient === auth.user._id && disconnect){
            ...
            const msg = {
                ...
                text: '', 
                media: [],
                call: {video: data.video, times},
                createdAt: new Date().toISOString()
            }

            dispatch(addMessage({msg, peer: {...call, _id: data.sender}, auth, socket}));
        }

        else if(call.sender === auth.user._id){
            const msg = {
                sender: data.sender,
                recipient: data.recipient,
                text: '', 
            ...
            if(disconnect){
                msg.sender = data.recipient;
                msg.recipient = data.sender; 
            }

            dispatch(addMessage({msg, peer: call.peerUser, auth, socket}));
        }
