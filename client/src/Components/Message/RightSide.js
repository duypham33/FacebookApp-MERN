...
const RightSide = () => {
    ...
    const handleChangeMedia = e => {
        ...
        if(err) dispatch({ type: GLOBAL_TYPES.NOTIFY, payload: {error: err} });
    ...
    const handleSubmit = async e => {
        ...
        setLoadMedia(false);
        await dispatch(addMessage({msg, peer: user, auth, socket}));
    ...
    useEffect(() => {
        const getMessagesData = async () => {
            if(message.data.every(item => item._id !== id)){
                const peer = message.users.find(u => u._id === id)
                if(peer)
                    await dispatch(getMessages({auth, conversation: peer.conversation, id: id}));
    ...
    //     if(isLoadMore > 1){
    //         if(result >= page * 9){
    //             dispatch(getMessages({auth, conversation: user.conversation, id: id, page: page + 1}));
    //             setIsLoadMore(1);
    //         }
    //     }
